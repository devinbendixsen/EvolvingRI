...
for (fileName in fileNames) {
  ...
  amp_count = nrow(cnv$gain)
  del_count = nrow(cnv$loss)
  ...
  cnv_summary$amp_count = amp_count
  cnv_summary$del_count = del_count
  cnv_summary$total_count = amp_count + del_count
...
# ==============================================================================
ggplot(H1) +
  geom_point(aes(x=amp_count,y=amp))
# ==============================================================================
# PLOT CORRELATIONS BETWEEN GENERATIONAL TIME AND MUTATIONAL BURDEN
# ==============================================================================
hybrid <- subset(cnv_data, (env =='H1F1' | env =='H1F2'|env =='H2F1' | env =='H2F2'|env =='H3F1' | env =='H3F2'))
hybrid$gen_num <- as.numeric(sub('G','',hybrid$gen))
hybrid$env <- sub('H.','',hybrid$env)

hybrid_corr <- ggplot(hybrid, aes(x=gen_num, y=total_count)) + 
...
H1_corr <- ggplot(H1, aes(x=gen_num, y=total_count)) + 
  geom_smooth(aes(color=env),method=lm, se=FALSE, fullrange=TRUE)+
...
H2_corr <- ggplot(H2, aes(x=gen_num, y=total_count)) + 
  geom_smooth(aes(color=env),method=lm, se=FALSE, fullrange=TRUE)+
...
H3_corr <- ggplot(H3, aes(x=gen_num, y=total_count)) + 
  geom_smooth(aes(color=env),method=lm, se=FALSE, fullrange=TRUE)+
  stat_cor(aes(color = env),label.x.npc = 0.4,label.y.npc = 0.25,p.accuracy=0.01,r.accuracy=0.01,size=2.5)+
  geom_point(alpha=alpha,aes(fill=env),shape=21, colour='black')+
  scale_color_manual(values=c(NaCl,Ethanol,F1,F2),name ='',labels = c("NaCl", "Ethanol", "Hybrid F1",'Hybrid F2'))+ 
...
ggsave('figures/Hybrid_Dynamics_CNV_burden_corr.pdf',width=5.5,height=2.5,dpi = 900)
# ==============================================================================
# PLOT CNV COUNTS
# ==============================================================================
...
H1$env <- factor(H1$env , levels=c('NaCl','LiAc0.01','H1F1','H1F2'))
H1_plot <- ggplot(H1) + 
  geom_hline(yintercept=(mean(founder$total_count)),color='black',linetype='dashed',linewidth=0.5)+
  stat_summary(aes(x=as.factor(env),y=total_count,color=env),fun=median, geom="point", shape=95, size=9,stroke=edgewidth,alpha=1)+
  geom_jitter(aes(x=as.factor(env),y=total_count,fill=env),shape=21, colour='black',size=markersize, alpha=alpha,width=width,stroke=edgewidth) +
  facet_wrap(~factor(gen,c('G100','G200','G300','G400','G500','G700','G1000')), ncol=7)+
  scale_fill_manual(values=c(NaCl,LiAc0.01,F1,F2),name ='',labels = c("NaCl", "LiAc0.01", "Hybrid F1",'Hybrid F2'))+
  scale_color_manual(values=c(NaCl,LiAc0.01,F1,F2),name ='',labels = c("NaCl", "LiAc0.01", "Hybrid F1",'Hybrid F2'))+
  theme(axis.title.x=element_blank(), 
...
H1_total <- ggplot(H1) + 
  geom_hline(yintercept=(mean(founder$total_count)),color='black',linetype='dashed',linewidth=0.5)+
  geom_boxplot(aes(x=as.factor(env),y=total_count,fill=env)) +
  scale_fill_manual(values=c(NaCl,LiAc0.01,F1,F2),name ='',labels = c("NaCl", "LiAc0.01", "Hybrid F1",'Hybrid F2'))+
  scale_color_manual(values=c(NaCl,LiAc0.01,F1,F2),name ='',labels = c("NaCl", "LiAc0.01", "Hybrid F1",'Hybrid F2'))+
  theme(axis.title.x=element_blank(), 
        axis.text.x=element_blank(),
...
ggsave('figures/Hybrid_Dynamics_CNV_load.pdf',width=8,height=4,dpi = 900)

# ==============================================================================
# Pairwise Kruskal-Wallis
# ==============================================================================
kruskal.test(total_count ~ env, data = H1)
pairwise.wilcox.test(H1$total_count, H1$env,
                     p.adjust.method = "BH")
kruskal.test(total_count ~ env, data = H2)
pairwise.wilcox.test(H2$total_count, H2$env,
                     p.adjust.method = "BH")
kruskal.test(total_count ~ env, data = H3)
pairwise.wilcox.test(H3$total_count, H3$env,
                     p.adjust.method = "BH")

H1 %>%
  group_by(env) %>%
  summarise(mean=mean(total_count),n = n())
H2 %>%
  group_by(env) %>%
  summarise(mean=mean(total_count),n = n())
H3 %>%
...
# ==============================================================================
# ==============================================================================
# IMPORT SOMATIC VCFS AND EXTRACT NEEDED DATA - KEEP AND COUNT ONLY LARGE VARIANTS
# ==============================================================================
fileNames <- Sys.glob("results/FREEC/*_CNVs.p.value.txt") # identify all samples
cnv_data <- data.frame()
for (fileName in fileNames) {
  ...
  cnv_sample$length <- (cnv_sample$end - cnv_sample$start)
  cnv_sample <- subset(cnv_sample,Wilcoxon<0.05 & KS <0.05)
  cnv_sample <- subset(cnv_sample, length>10000)
...
(H1_small+H2_small+H3_small) / (H1_total+H2_total+H3_total) / (H1_aneu+H2_aneu+H3_aneu)
ggsave('figures/Hybrid_Dynamics_CNV_size_dist.pdf',width=4,height=2.5,dpi = 900)
